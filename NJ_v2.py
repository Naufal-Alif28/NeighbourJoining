...
def BranchLen(tree, imin, jmin, dist_matrix, r_vector):
    ...
    if N == 2:
        ...
        x = 2
    brlen_i = 0.5*(dist_matrix[imin][jmin]) + (1/(2*(N-x))*(r_vector[imin] - r_vector[jmin]))
...
def ModifTree(tree, dist_matrix_adj):
    ...
    brlen = BranchLen(
        tree,imin,jmin,dist_matrix,r_vector
    )
